...
#define CALL_PA_DECODE_DOT_KERNEL(head_sz, grp_sz, partition_sz, output_ptr) \
    HIP_CHECK(_pa_decode_dot_kernel##head_sz##_##grp_sz##_##partition_sz( \
        stream, \
        exp_sums_ptr, \
        max_logits_ptr, \
        output_ptr, \
...
// 定义宏来调用 reduce kernel
#define CALL_PA_DECODE_REDUCE_KERNEL(head_sz, grp_sz, partition_num) \
    HIP_CHECK(_pa_decode_reduce_kernel##head_sz##_##grp_sz##_##partition_num( \
        stream, \
        out_ptr, \
        exp_sums_ptr, \
        max_logits_ptr, \
...
// 根据 head_size, grp_size, partition_size 选择对应的 dot kernel
    ...
    do { \
        if (head_sz == 64 && grp_sz == 2 && partition_sz == 256) { \
            ...
            throw std::runtime_error("Unsupported combination: head_size=" + std::to_string(head_sz) + \
                                   ...
                                   ", partition_size=" + std::to_string(partition_sz)); \
        } \
    } while(0)

// 根据 head_size, grp_size, partition_num 选择对应的 reduce kernel
